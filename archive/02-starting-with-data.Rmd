...
We are going to use the R function `read.csv()` to load the content of the CSV file into R. 
...
-->
**\<note>** check here that everyone has this file if you prefer to use the download method 
...
Or load it via the ratdat package
...
semicolon (;) is used as a field delineator. If you want to read in files with other structures check out `?read.table` for more.
...
\<note>
Working with factors and converting factors was moved to extra lessons
\<\note>
