...
#source("setup.R")
#surveys_complete <- read_csv(file = "data/portal_data_joined.csv", col_types = cols()) %>% drop_na(weight, hindfoot_length, sex) %>% group_by(species_id) %>% filter(n() >= 50) %>% ungroup()
#write_csv(surveys_complete,'data/portal_surveys_complete.csv')
```
...
surveys_complete <- read_csv("data/portal_surveys_complete.csv")
```
